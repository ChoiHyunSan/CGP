...
void Stage01_Scene::init(D3DClass* D3D)
{
	...
	if (m_BumpMapShader == nullptr)
	{
		// Create the bump map shader object.
		m_BumpMapShader = new BumpMapShaderClass;
		result = m_BumpMapShader->Initialize(D3D->GetDevice(), SystemClass::GetInst()->GetHwnd());
		if (!result)
		{
			MessageBox(SystemClass::GetInst()->GetHwnd(), L"Could not initialize the bump map shader object.", L"Error", MB_OK);
		}
	}
